...
// Water temperature safe range (for aquaculture, drinking water, etc.)
const float waterTempSafeHigh = 35.0;  // deg C (above this may harm aquatic life or indicate system issue)
const float waterTempSafeLow  = 10.0;  // deg C (below this may harm aquatic life or indicate freezing risk)

// TDS (Total Dissolved Solids) safe range in ppm
const float tdsSafeHigh = 500.0;   // ppm (above this water quality is poor for drinking)
const float tdsSafeLow  = 50.0;    // ppm (below this water may be too pure/demineralized)

// EC (Electrical Conductivity) safe range in µS/cm
const float ecSafeHigh = 1000.0;   // µS/cm (correlates with TDS ~500 ppm)
const float ecSafeLow  = 100.0;    // µS/cm (correlates with TDS ~50 ppm)

// TDS sensor calibration defaults (can be adjusted at runtime via AppState)
const float tdsVRef = 5.0;        // Reference voltage (5V power rail)
const float tdsFactor = 0.5;      // TDS conversion factor (adjust via calibration)
const int tdsSampleCount = 30;    // Number of samples for averaging
